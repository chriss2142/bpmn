 ...
 *
 * Updated: Now supports additional persistency implementations:
 * - PostgreSQL (using "postgres://")
 * - Supabase (using "supabase://")
 * - Redis (using "redis://")
 *
 * The Persistency module automatically selects the proper implementation
 * based on the URI scheme provided in the persistencyOptions.
 */
"use strict";
 ...
 *      persistencyOptions: {uri: String, doneLoading: Function, doneSaving: Function}=  // Note: uri now supports:
 *           - "path/to/folder" for file persistency,
 *           - "mongodb://..." for MongoDB,
...
var ProcessManager = exports.ProcessManager = function(options){
    ...
    if (options.persistencyOptions) {
        // The Persistency module will pick the correct implementation
        // based on the persistencyOptions.uri scheme.
...
/**
 * Initialise a new definition by loading all persisted processes.
 * All other functions that need the initialise state will wait until initialization is done.
 *
...
ProcessManager.prototype._initialiseDefinition = function(processDefinition){
    ...
    function next(){
        ...
        self._persistency.loadAll(currentDefinition.name, function(err, documents){
            if(err){
                execCallbacks(err);
                return;
            }
            ...
            async.each(documents, function(document, done){
                self._createSingleProcess(document.processId, currentDefinition.name, function(err, bpmnProcess){
                    if(err){
                        return done(err);
                    }
 ...
 * All process definitions found in this file will be initialized and replace the old ones if exists.
 ...
 * All process definitions found in this XML will be initialized and replace the old ones if exists.
 * A process handler object or file path can be passed. If none passed the same file path as the bpmn is used or the existing handler.
 * An error is thrown if no handler is found.
 *
 ...
 */
ProcessManager.prototype.get = function(processId, callback) {
    var self = this;
...
/**
 * A BPMN process is created using the descriptor name and id.
 * If there are multiple definitions, an array of descriptors must be passed and collaborating processes are created.
